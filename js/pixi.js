...
loader
	.add([
		...
		{name:'battle_theme2', url:'sfx/Ariadne-battle01_loop.ogg'},
...
enemy[1] = {
	...
	skill:[4,3,6,22],
	items: [2,2],
	statDODG: 0, statHP: 82, statPATK: 0, statPDEF: 0, statSATK: 0, statSDEF: 0, statSPD: 0,
	hero: false
};
enemy[2] = {
	id: 13, level: 45,
...
function onInfoDown(){
	...
	if(this.identifier[0] == 0){
		...
		// creatureInfo.statusIcon = [];
		creatureInfo.statusIcon.length = 0;
...
function onHPDown(){
	...
	creatureInfo.info_stat_text.forEach((text,textIndex) =>{
		...
		if(textIndex > 27){
			text.x = (textIndex-27) * app.screen.width/10 + app.screen.width/20;
		}else{
			text.x = (textIndex%4) * app.screen.width/10 + app.screen.width/20;
				...
				list.forEach((creatureObject, creatureIndex)=>{
					...
					if(skillList.data.skill[skillID].type == "Physical"){
						...
						if(heal){
							...
							damageCalc = skillList.data.skill[skillID].heal * skillAccuracy * columnMod * (1+(hpWeight*2));
							weight.push(healMod);
							// damageCalc *= healMod;
							// weight.push(healMod);	
						}
